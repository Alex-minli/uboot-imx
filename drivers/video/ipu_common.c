...
// minli-debug_video
	debug("\t parent_rate = clk->parent->rate = %lldHz\n", parent_rate);
	debug("\t rate = %d\n\n", rate);

	remainder = do_div(div, rate);

// minli-debug_video
	debug("\t calculate div: parent_rate / rate. div = %lld, remainder = %d\n", div, remainder);
...
// minli-debug_video
	debug("\t Round Div to %lld\n\n", div);

// minli-debug_video
	debug("\t Recalculate final rate = parent_rate / Div.\n");
	debug("\t parent_rate = clk->parent->rate = %lldHz\n", parent_rate);
	debug("\t div = %lld\n\n", div);
	do_div(final_rate, div);

// minli-debug_video
	debug("\t final_rate = %lld\n\n", final_rate);
	return final_rate;
}
...
// minli-debug_video
	debug("\tDI_BS_CLKGEN0(%d)(0x%08x) = 0x%x(%lld)\n", clk->id, DI_BS_CLKGEN0(clk->id), (u32)div, div);
	__raw_writel(div, DI_BS_CLKGEN0(clk->id));

	/* Setup pixel clock timing */
	/* Down time is half of period */
// minli-debug_video
	debug("\tDI_BS_CLKGEN1(%d)(0x%08x) = 0x%x\n", clk->id, DI_BS_CLKGEN1(clk->id), (div / 16) << 16);
...
// minli-debug_video
	if (parent == g_ipu_clk) {
		debug("\t -> parent is g_ipu_clk: The DI clock is internally generated by the IPU.\n");
		di_gen &= ~DI_GEN_DI_CLK_EXT;
#if defined(CONFIG_MX6) || defined(CONFIG_MX53)
	} else if (!IS_ERR(g_di_clk[clk->id]) && parent == g_ldb_clk[clk->id]) {
		debug("\t -> parent is g_ldb_clk: The source of the DI clock is external to the IPU.\n");
		di_gen |= DI_GEN_DI_CLK_EXT;
#endif
	} else
		return -EINVAL;
...
// minli-debug_video
	debug("\t DI_GENERAL(%d)(0x%p) = %08x\n", clk->id, DI_GENERAL(clk->id), di_gen);
...
{
// minli-debug_video
	debug("%s: Software reset for IPU1\n", __func__);
...
// minli-debug_video
	debug("\t ipu_base = 0x%08x, ipu_cpmem_base = 0x%08x, ipu_dc_tmpl_reg = 0x%08x\n", ipu_base, ipu_cpmem_base, ipu_dc_tmpl_reg);
...
// minli-debug_video
	debug("\t Set pixel_clk 0 & 1's parent to IPU clock\n");
 ...
 */
	...
	debug("\t IPU_CONF (0x%08X)             = \t0x%08X\n", IPU_CONF,		 __raw_readl(IPU_CONF));
	debug("\t IDMAC_CONF (0x%08X)           = \t0x%08X\n", IDMAC_CONF,		 __raw_readl(IDMAC_CONF));
	debug("\t IDMAC_CHA_EN1 (0x%08X)        = \t0x%08X\n", IDMAC_CHA_EN(0),		 __raw_readl(IDMAC_CHA_EN(0)));
	debug("\t IDMAC_CHA_EN2 (0x%08X)        = \t0x%08X\n", IDMAC_CHA_EN(32),	 __raw_readl(IDMAC_CHA_EN(32)));
	debug("\t IDMAC_CHA_PRI1 (0x%08X)       = \t0x%08X\n", IDMAC_CHA_PRI(0),	 __raw_readl(IDMAC_CHA_PRI(0)));
	debug("\t IDMAC_CHA_PRI2 (0x%08X)       = \t0x%08X\n", IDMAC_CHA_PRI(32),	 __raw_readl(IDMAC_CHA_PRI(32)));
	debug("\t IPU_CHA_DB_MODE_SEL0 (0x%08X) = \t0x%08X\n", IPU_CHA_DB_MODE_SEL(0),	 __raw_readl(IPU_CHA_DB_MODE_SEL(0)));
	debug("\t IPU_CHA_DB_MODE_SEL1 (0x%08X) = \t0x%08X\n", IPU_CHA_DB_MODE_SEL(32),	 __raw_readl(IPU_CHA_DB_MODE_SEL(32)));
	debug("\t DMFC_WR_CHAN (0x%08X)         = \t0x%08X\n", DMFC_WR_CHAN,	 	 __raw_readl(DMFC_WR_CHAN));
	debug("\t DMFC_WR_CHAN_DEF (0x%08X)     = \t0x%08X\n", DMFC_WR_CHAN_DEF,	 __raw_readl(DMFC_WR_CHAN_DEF));
	debug("\t DMFC_DP_CHAN (0x%08X)         = \t0x%08X\n", DMFC_DP_CHAN,		 __raw_readl(DMFC_DP_CHAN));
	debug("\t DMFC_DP_CHAN_DEF (0x%08X)     = \t0x%08X\n", DMFC_DP_CHAN_DEF,       	 __raw_readl(DMFC_DP_CHAN_DEF));
	debug("\t DMFC_IC_CTRL (0x%08X)         = \t0x%08X\n", DMFC_IC_CTRL,	     	 __raw_readl(DMFC_IC_CTRL));
	debug("\t IPU_FS_PROC_FLOW1 (0x%08X)    = \t0x%08X\n", IPU_FS_PROC_FLOW1,	 __raw_readl(IPU_FS_PROC_FLOW1));
	debug("\t IPU_FS_PROC_FLOW2 (0x%08X)    = \t0x%08X\n", IPU_FS_PROC_FLOW2,	 __raw_readl(IPU_FS_PROC_FLOW2));
	debug("\t IPU_FS_PROC_FLOW3 (0x%08X)    = \t0x%08X\n", IPU_FS_PROC_FLOW3,	 __raw_readl(IPU_FS_PROC_FLOW3));
	debug("\t IPU_FS_DISP_FLOW1 (0x%08X)    = \t0x%08X\n", IPU_FS_DISP_FLOW1,	 __raw_readl(IPU_FS_DISP_FLOW1));
}
...
// minli-debug_video
	debug("EBA0 0x%08x\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ch), 1, 0, 29));
	debug("EBA1 0x%08x\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ch), 1, 29, 29));
...
{
// minli-debug_video
	debug("CPMEM setting value of CH %d\n", channel_2_dma(channel, type));
